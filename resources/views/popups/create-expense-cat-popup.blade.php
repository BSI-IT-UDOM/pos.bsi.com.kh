<!-- Popup form -->
<div id="createExpenseCatePopup" class="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center hidden z-60">
    <div class="bg-white rounded-lg shadow-lg max-w-xl w-full max-h-screen overflow-y-auto">
        <div class="bg-gradient-to-b from-blue-500 to-blue-400 rounded-t-lg px-6 py-4">
            <h2 class="text-2xl font-bold text-white mb-2">NEW EXPENSE CATEGORY</h2>
        </div>
        <form id="expenseCateForm" action="{{ route('createExpenseCate') }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            <div class="mb-4">
            ...
            <div class="text-end">
                ...
                <button type="button" id="closeCreateExpenseCatePopup" class="bg-gray-300 hover:bg-gray-400 text-gray-900 px-4 py-2 rounded-md ml-2 focus:outline-none">CANCEL</button>
            </div>
        </form>
    </div>
  </div>
<script>
    document.getElementById('closeCreateExpenseCatePopup').addEventListener('click', function() {
        document.getElementById('createExpenseCatePopup').classList.add('hidden');   
        document.getElementById('expenseCateForm').reset();    
